...
class lya_convert(object):
    # this class defines the convertor for our fiducial cosmology, remember that this normally should not depend on cosmology since one usually ignore the change of the covariance with cosmology (see the verde & bernal paper (2020) on forecasts errors). However, since we change Omega_dm, then it will need to know which density are we using.
    
    def __init__(self,Omega_dm):
        # need to define cosmology for hubble parameter
        self.Omega_dm = Omega_dm
        # baryon density, which we do not change is
        Omega_b = 0.0486
        # then the total matter density is
        self.Omega_m = Omega_b + Omega_dm
        self.h = 0.6774
