...
"""
    Build the HI density and we want to pickle this function to save time!
...
directory = '/scratch/yaozhang/%s/%s/'%(realization,model)
...
params['m_wdm'] = 1.0 / float(sys.argv[3])
params['sigma8'] = float(sys.argv[4])
...
for z_re in z_res:
    ...
    for i in range(len(z_cs)):
        ...
        for m in Mhalos:
            R=(3.*m/4./np.pi/rhom)**(1./3)   #Yao: in Mpc
