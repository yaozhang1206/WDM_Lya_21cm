...
for z in z_bin:
    ...
    mu_bin, dmu_bin = bin_class.mu_bins()
    ...
    for k, dk in zip(k_bin, dk_bin):
        for mu in mu_bin:
            ...
            one_bin[0,0] = wdm_3keV_sminus.Total_P_21_Mpc_norm(z, k, mu)
            one_bin[0,1] = wdm_4keV_sminus.Total_P_21_Mpc_norm(z, k, mu)
            one_bin[0,2] = wdm_6keV_sminus.Total_P_21_Mpc_norm(z, k, mu)
            one_bin[0,3] = wdm_9keV_sminus.Total_P_21_Mpc_norm(z, k, mu)
            one_bin[0,4] = cdm_sminus.Total_P_21_Mpc_norm(z, k, mu)
            one_bin[1,0] = wdm_3keV_s8.Total_P_21_Mpc_norm(z, k, mu)
            one_bin[1,1] = wdm_4keV_s8.Total_P_21_Mpc_norm(z, k, mu)
            one_bin[1,2] = wdm_6keV_s8.Total_P_21_Mpc_norm(z, k, mu)
            one_bin[1,3] = wdm_9keV_s8.Total_P_21_Mpc_norm(z, k, mu)
            one_bin[1,4] = cdm_s8.Total_P_21_Mpc_norm(z, k, mu)
            one_bin[2,0] = wdm_3keV_splus.Total_P_21_Mpc_norm(z, k, mu)
            one_bin[2,1] = wdm_4keV_splus.Total_P_21_Mpc_norm(z, k, mu)
            one_bin[2,2] = wdm_6keV_splus.Total_P_21_Mpc_norm(z, k, mu)
            one_bin[2,3] = wdm_9keV_splus.Total_P_21_Mpc_norm(z, k, mu)
            one_bin[2,4] = cdm_splus.Total_P_21_Mpc_norm(z, k, mu)
            bins_inter.append(interpolate.interp2d(inverse_mass, sigma8, one_bin))
            ref_bin.append(cdm_s8.Total_P_21_Mpc_norm(z, k, mu))
            var_bin.append(cdm_s8.Var_autoHI_Mpc(z, k, mu, epsilon)) # Yao: note this func assume dmu=0.2, which might need modification
