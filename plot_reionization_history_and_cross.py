...
ax1.plot(z,xH_4keV,color='salmon',label=r'm$_{\rm wdm} = 4$ keV',zorder=4)
...
cross_4kev = interp2d(z, k, cross_4keV_a, kind='cubic')
...
for i in range(0,len(z)):
    plot_3keV[i] = cross_3kev(z[i], k_test)
    plot_4keV[i] = cross_4kev(z[i], k_test)
    plot_6keV[i] = cross_6kev(z[i], k_test)
    plot_9keV[i] = cross_9kev(z[i], k_test)
    plot_cdm[i] = cross_cdm(z[i], k_test)
...
for i in range(0,len(k)):
    plot_3keV[i] = cross_3kev(z_50_3keV, k[i])
    plot_4keV[i] = cross_4kev(z_50_4keV, k[i])
    plot_6keV[i] = cross_6kev(z_50_6keV, k[i])
    plot_9keV[i] = cross_9kev(z_50_9keV, k[i])
    plot_cdm[i] = cross_cdm(z_50_cdm, k[i])
# and plot
fig = plt.figure(figsize=(6,6))
ax1 = fig.add_subplot(111)
ax1.plot(k,plot_cdm,color='black',label=r'cdm')
ax1.plot(k,plot_3keV,color='purple',label=r'm$_{\rm wdm} = 3$ keV')
ax1.plot(k,plot_4keV,color='salmon',label=r'm$_{\rm wdm} = 4$ keV')
ax1.plot(k,plot_6keV,color='aqua',label=r'm$_{\rm wdm} = 6$ keV')
ax1.plot(k,plot_9keV,color='chartreuse',label=r'm$_{\rm wdm} = 9$ keV')
ax1.grid(linestyle='dotted')
...
plot_cdm = np.zeros(len(k))
for i in range(0,len(k)):
    plot_3keV[i] = cross_3kev(z_25_3keV, k[i])
    plot_4keV[i] = cross_3kev(z_25_4keV, k[i])
    plot_6keV[i] = cross_6kev(z_25_6keV, k[i])
    plot_9keV[i] = cross_9kev(z_25_9keV, k[i])
    plot_cdm[i] = cross_cdm(z_25_cdm, k[i])
# and plot
fig = plt.figure(figsize=(6,6))
ax1 = fig.add_subplot(111)
ax1.plot(k,plot_cdm,color='black',label=r'cdm')
...
plot_cdm = np.zeros(len(k))
for i in range(0,len(k)):
    plot_3keV[i] = cross_3kev(z_75_3keV, k[i])
    plot_4keV[i] = cross_3kev(z_75_4keV, k[i])
    plot_6keV[i] = cross_6kev(z_75_6keV, k[i])
    plot_9keV[i] = cross_9kev(z_75_9keV, k[i])
    plot_cdm[i] = cross_cdm(z_75_cdm, k[i])
# and plot
fig = plt.figure(figsize=(6,6))
ax1 = fig.add_subplot(111)
ax1.plot(k,plot_cdm,color='black',label=r'cdm')
