...
# run mcmc chain
filename = "chain_21cm_%s_%s_f.h5"%(tele,realization)
...
fig.savefig('chain_21cm_%s_%s_f.pdf'%(tele,realization))


samples = sampler.get_chain(flat=True, discard=2000, thin=20)
fig1 = corner.corner(
    samples, labels=['1keV / m', r'$\sigma_8$'], truths=[0, 0.8159])
fig1.savefig('corner_21cm_%s_%s_f.pdf'%(tele,realization))
