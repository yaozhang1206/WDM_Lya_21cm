...
# run mcmc chain
filename = "chain_combine_%s_%s_f.h5"%(tele,realization)
new_backend = emcee.backends.HDFBackend(filename)
new_sampler = emcee.EnsembleSampler(nwalkers = nw, ndim = nd, log_prob_fn = log_prob, backend=new_backend, moves=emcee.moves.StretchMove(a=4.0))
new_sampler.run_mcmc(None, 50000, progress=False)
...
fig.savefig('chain_combine_%s_%s_f.pdf'%(tele,realization))


samples = sampler.get_chain(flat=True, discard=2000, thin=20)
fig1 = corner.corner(
    samples, labels=['1keV / m', r'$\sigma_8$'], truths=[0, 0.8159])
fig1.savefig('corner_combine_%s_%s_f.pdf'%(tele,realization))
