...
class observed_3D(object):
    ...
    """ No need to compute this for anything else than CDM REFERENCE (so not even changing sigma 8 or omega M) so, let's reflect that """
    
    def __init__(self, band):
        # cosmology, important for models
        self.Omega_b = 0.0486
        self.Omega_dm = 0.2602
        self.Omega_m = self.Omega_dm + self.Omega_b
        # note that we only need the reference model so no need to change sigma8 here
        self.sigma_8 = 0.8159
        self.band = band
        self.h = 0.6774
        self.c_kms = 2.998e5
        ...
        # wavelength range being covered (amrstrongs), this first one corresponds to z~2
...
#        self.kp_w_kms = 0.01
        ...
        self.verbose = 0
        # luminosity function
        """ Now QLF needs to choose the default one (0) or the 'SV' leaked by Yeche (1), keep it to 0 for now, I will change this if the QLF gets released to the public """
        self.QLF = lumin.QSO_LF()
        # spectrograph
        self.my_pix = pix.spec_pixel(self.band)
        # convertor
        self.convert = convertor.lya_convert(self.Omega_dm)
        # theory
        print('Building reference model for variance \n')
        self.my_P = theory.theory_P_lyas(np.infty, self.sigma_8, '21cm_avg_cdm', 'gadget_ave_cdm')
    ...
    def FluxP1D_kms(self,kp_kms):
        """1D Lya power spectrum in observed coordinates, smoothed with pixel width and resolution in CDM """
        z = self.mean_z()
    ...
    def FluxP3D_Mpc(self,kt_deg,kp_kms):
        ...
        P_Mpc = self.my_P.FluxP3D_lya_Mpc(z,k_Mpc,mu)
        # adding reio
        P_Mpc = P_Mpc + self.my_P.FluxP3D_reio_Mpc(z,k_Mpc,mu)
        # back to funny units for smoothing
...
#        print('P_degkms = ',P_degkms)
    ...
    def EffectiveDensityAndNoise(self):
        ...
        """ Warning for Yao: this takes time, but it also only depends on redshift, so make sure to compute it once per redshift, this is why Total Flux and variance accept Pw2D and PN_eff as arguments! """
...
#        print('PT_degkms, dkms_dMpc, dMpc_ddeg = ', PT_degkms, dkms_dMpc, dMpc_ddeg)
    ...
    def VarFluxP3D_Mpc(self, k_Mpc,mu,dk_Mpc,dmu,Pw2D=None,PN_eff=None):
        ...
        Nmodes = V_Mpc * k_Mpc * k_Mpc * dk_Mpc * dmu / (2.0 * np.pi * np.pi)
